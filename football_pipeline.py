...
def build_features_and_labels(df, elo):
    ...
    for _, row in df.iterrows():
        ...
        agc = (
            sum(aws["goals_conceded"][-10:]) / max(len(aws["goals_conceded"][-10:]), 1)
            if aws["goals_conceded"]
            else 1.1
        )
        ...
        features = [
            ...
            agc,
            (hgs - agc),
            (ags - hgc),  # attack vs defense matchup
            h_shots,
            a_shots,
            h_shots - a_shots,
            h_sot,
...
def build_football_models():
    ...
    if HAS_LGB:
        m["LGBM"] = lgb.LGBMClassifier(n_estimators=100, max_depth=5, learning_rate=0.1, random_state=42, verbose=-1)
    return m
...
def predict_upcoming(models, scaler, elo, team_stats, h2h_record, upcoming):
    ...
    for match in upcoming:
        ...
        home_form = (
            sum(hs.get("points", [1])[-5:]) / max(len(hs.get("points", [1])[-5:]), 1) if hs.get("points") else 1.0
        )
        away_form = (
            sum(aws.get("points", [1])[-5:]) / max(len(aws.get("points", [1])[-5:]), 1) if aws.get("points") else 1.0
        )
        hgs = (
            sum(hs.get("goals_scored", [1.3])[-10:]) / max(len(hs.get("goals_scored", [])[-10:]), 1)
            if hs.get("goals_scored")
            else 1.3
        )
        hgc = (
            sum(hs.get("goals_conceded", [1.1])[-10:]) / max(len(hs.get("goals_conceded", [])[-10:]), 1)
            if hs.get("goals_conceded")
            else 1.1
        )
        ags = (
            sum(aws.get("goals_scored", [1.3])[-10:]) / max(len(aws.get("goals_scored", [])[-10:]), 1)
            if aws.get("goals_scored")
            else 1.3
        )
        agc = (
            sum(aws.get("goals_conceded", [1.1])[-10:]) / max(len(aws.get("goals_conceded", [])[-10:]), 1)
            if aws.get("goals_conceded")
            else 1.1
        )
        h_shots = sum(hs.get("shots", [12])[-10:]) / max(len(hs.get("shots", [])[-10:]), 1) if hs.get("shots") else 12
        a_shots = (
            sum(aws.get("shots", [12])[-10:]) / max(len(aws.get("shots", [])[-10:]), 1) if aws.get("shots") else 12
        )
        ...
        a_cor = (
            sum(aws.get("corners", [5])[-10:]) / max(len(aws.get("corners", [])[-10:]), 1) if aws.get("corners") else 5
        )
        ...
        predictions.append(
            {
                ...
                "goals_avg": {
                    "home_scored": round(hgs, 2),
                    "home_conceded": round(hgc, 2),
                    "away_scored": round(ags, 2),
                    "away_conceded": round(agc, 2),
                },
            }
        )
...
def main():
    ...
    for p in predictions:
        ...
        print(
            f"  │    Oracle: H {o['home_pct']:>5.1f}% D {o['draw_pct']:>5.1f}% A {o['away_pct']:>5.1f}%  "
            f"→ {icon} {result:<15} [{o['confidence']}] │"
        )
        print(
            f"  │    Market: H {sr.get('H', 0):>5.1f}% D {sr.get('D', 0):>5.1f}% A {sr.get('A', 0):>5.1f}%  "
    ...
    try:
        ...
        for p in predictions:
            ...
            if best["ev"] > 0:
                ...
                print(f"    {p['match']}")
                print(
                    f"      {side}: EV={best['ev']:+.3f} | Edge={best['edge_pct'] * 100:.1f}% | Kelly={kelly * 100:.1f}%"
                )
...
def poisson_score_predict(
    home_goals_avg, home_conceded_avg, away_goals_avg, away_conceded_avg, league_avg_goals=1.35, max_goals=6, elo_diff=0
):
...
def backtest_roi_on_training_data(df, elo, team_stats, models, scaler):
    ...
    try:
        from analytics import BankrollSimulator
