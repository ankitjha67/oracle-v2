...
class PlayerDatabase:
    ...
    CRICKET_PLAYERS = {  # noqa: RUF012
        ...
        "Hardik Pandya": {
            ...
            "vs": {"NZ": {"avg": 30, "sr": 155}},
            "venue": {"Ahmedabad": {"avg": 35}},
            "form_last5": [43, 30, 15, 22, 38],
            "fitness": 82,
        },
        "Suryakumar Yadav": {
            "team": "India",
            ...
            "vs": {"NZ": {"avg": 38, "sr": 162}},
            "venue": {"Ahmedabad": {"avg": 40, "sr": 170}},
            "form_last5": [34, 28, 52, 18, 45],
            "fitness": 88,
        },
        "Varun Chakravarthy": {
            "team": "India",
        ...
        "Rachin Ravindra": {
            ...
            "impact": 75,
            "wc_wickets": 7,
            "form_last5": [2, 3, 0, 1, 1],
            "fitness": 92,
        },
    }

    @classmethod
    def get_player(cls, name: str) -> dict | None:
        return cls.CRICKET_PLAYERS.get(name)

    @classmethod
    def get_team_players(cls, team: str) -> list[dict]:
        return [{"name": k, **v} for k, v in cls.CRICKET_PLAYERS.items() if v.get("team") == team]

    @classmethod
    def team_aggregate_stats(cls, team: str, opponent: str = "", venue: str = "") -> dict:
        ...
        if not players:
            return {
                "avg_impact": 50,
                "batting_form_avg": 25,
                "bowling_form_avg": 1.5,
                "vs_opponent_bat_avg": 50,
                "vs_opponent_bowl_econ": 7.5,
                "venue_batting_avg": 50,
                "fitness_avg": 85,
                "powerplay_sr": 135,
                "death_bowling_econ": 8.5,
                "player_count": 0,
                "top_impact_player": 50,
                "weakest_player_impact": 50,
            }
        ...
        if opponent:
            vs_avgs = [
                p.get("vs", {}).get(opponent, {}).get("avg", 0)
                for p in players
                if p.get("vs", {}).get(opponent, {}).get("avg")
            ]
            if vs_avgs:
                vs_batting_avg = sum(vs_avgs) / len(vs_avgs)
            vs_econs = [
                p.get("vs", {}).get(opponent, {}).get("econ", 0)
                for p in players
                if p.get("vs", {}).get(opponent, {}).get("econ")
            ]
            if vs_econs:
        ...
        if venue:
            ...
            for p in players:
                for vk, vs in p.get("venue", {}).items():
                    if v_key in vk.lower() and "avg" in vs:
...
def extract_features(match: dict, db: OracleDB = None, weather: dict | None = None) -> np.ndarray:
    """
    ...
    # --- Toss ---
    toss_adv = 0
    if match.get("toss_winner") == ta:
        toss_adv = 1
    elif match.get("toss_winner") == tb:
        toss_adv = -1
...
def build_models() -> dict[str, Any]:
    models = {
        ...
        "GradientBoosting": GradientBoostingClassifier(
            n_estimators=200, max_depth=5, learning_rate=0.05, random_state=42, subsample=0.8
        ),
        "LogisticRegression": LogisticRegression(C=1.0, max_iter=5000, random_state=42, class_weight="balanced"),
        "NeuralNetwork": MLPClassifier(
            hidden_layer_sizes=(64, 32, 16), max_iter=3000, random_state=42, learning_rate="adaptive"
        ),
...
class OracleV2:
    ...
    def load_historical_data(self, matches: list[dict]):
        ...
        for m in matches:
            ...
            if m.get("winner") and m["winner"] not in ("", "NO RESULT"):
                ...
                self.ratings.update_all(m["winner"], loser, m.get("sport", "cricket"), margin)
    ...
    def train(self, matches: list[dict]) -> dict:
        ...
        for name, model in self.models.items():
            try:
                ...
                scores = cross_val_score(model, X_scaled, y, cv=n_cv, scoring="accuracy")
        ...
        # Super ensemble
        estimators = [(n.replace(" ", "_")[:20], m) for n, m in self.models.items() if hasattr(m, "predict_proba")]
        if len(estimators) >= 2:
        ...
        if len(X_scaled) >= 10:
            ...
            if meta_features and len(meta_features[0]) >= 2:
                ...
                try:
                    ...
                    meta_cv = cross_val_score(self.meta_learner, meta_X, y, cv=min(5, len(y)), scoring="accuracy")
    ...
    def predict(self, match: dict) -> dict:
        ...
        confidence = (
            "VERY HIGH"
            if effective_diff > 0.45
            else "HIGH"
            if effective_diff > 0.30
            else "MODERATE"
            if effective_diff > 0.15
            else "LOW"
            if effective_diff > 0.05
            else "TOSS-UP"
        )
        ...
        if self._shap_explainer is not None:
            shap_explanation = self._shap_explainer.explain_prediction(self.models, features_scaled, top_n=5)
        ...
        if not feat_imp:
            ...
            if feat_imp:
                ...
                feat_imp = {k: round(v / total, 4) for k, v in sorted(feat_imp.items(), key=lambda x: -x[1])[:15]}

        # Confidence interval from bootstrap over model outputs
        ci_data = {}
        if len(probs_a) >= 3:
            rng = np.random.default_rng(42)
            boot_means = [float(np.mean(rng.choice(probs_a, size=len(probs_a), replace=True))) for _ in range(500)]
            ci_data = {
        ...
        if odds_prob_a > 0 and odds_prob_b > 0:
            try:
                ...
                analytics_data = {
                    ...
                    "value_side": match["team_a"]
                    if ev_a["ev"] > ev_b["ev"] and ev_a["ev"] > 0
        ...
        prediction = {
            ...
            "models_for_a": sum(1 for v in model_votes.values() if v["winner"] == match["team_a"]),
            "models_for_b": sum(1 for v in model_votes.values() if v["winner"] == match["team_b"]),
            "feature_importance": feat_imp,
            "shap_explanation": shap_explanation,
            "analytics": analytics_data,
            "weather": weather,
            "odds_market": {"prob_a": match.get("odds_prob_a", 0), "prob_b": match.get("odds_prob_b", 0)},
        }
        ...
        self.db.log_event(
            "prediction",
            {
                ...
                "winner": winner,
                "prob": round(calibrated_prob_a * 100, 1),
            },
        )
    ...
    def backtest(self, matches: list[dict], min_train: int = 10) -> BacktestResult:
        ...
        backtester = WalkForwardBacktester(
            model_factory=model_factory, feature_extractor=lambda m: m["features"], min_training_size=min_train
        )
        return backtester.run(matches)

    def monte_carlo(self, groups: dict[str, list[str]], n_sims: int = 10000) -> dict:
...
def main():
    ...
    print("\n  ┌─────────────────────────────────────────────────┐")
    print("  │  MODEL CROSS-VALIDATION ACCURACY                │")
    print("  └─────────────────────────────────────────────────┘")
    ...
    # ── Predict the Final ──
    print(f"\n  {'═' * 70}")
    print("  🏆 PREDICTING: INDIA vs NEW ZEALAND — T20 WC FINAL")
    print(f"  {'═' * 70}")
    ...
    final_pred = oracle.predict(
        {
            ...
            "toss_winner": "New Zealand",
            "toss_decision": "field",
            "form_a": [1, 1, 1, 0, 1, 1, 1],
            "form_b": [1, 1, 0, 1, 0, 1, 1],
            "streak_a": 3,
            "streak_b": 2,
            "odds_prob_a": 0.70,
            "odds_prob_b": 0.30,  # Market consensus
            "elo_a": 2195,
            "elo_b": 2040,
        }
    )
    ...
    print(f"  Models:       {final_pred['models_for_a']} for India | {final_pred['models_for_b']} for NZ")
    ...
    print(
        f"  Market Odds:  India {final_pred['odds_market']['prob_a'] * 100:.0f}% | "
    ...
    team_elos = {}
    for t in ["India", "South Africa", "West Indies", "Zimbabwe", "England", "New Zealand", "Pakistan", "Sri Lanka"]:
    ...
    print("\n  ┌─────────────────────────────────────────────────┐")
    print("  │  MONTE CARLO: TOURNAMENT WIN PROBABILITY        │")
    print("  └─────────────────────────────────────────────────┘")
    ...
    print("  📁 Saved: oracle_v2/backtest_results.json")
    print("  📁 Saved: oracle_v2/monte_carlo.json")
    print(f"\n{'═' * 80}")
