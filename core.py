...
class OracleDB:
    ...
    def insert_match(self, match: dict) -> str:
        mid = (
            match.get("id")
            or hashlib.md5(f"{match['team_a']}_{match['team_b']}_{match.get('date', '')}".encode()).hexdigest()[:16]
        )
        with self.transaction() as conn:
            conn.execute(
                """
    ...
    def get_matches(
        self, sport: str | None = None, team: str | None = None, limit: int = 1000, stage: str | None = None
    ) -> list[dict]:
        conn = self._get_conn()
    ...
    def get_h2h(self, team_a: str, team_b: str, sport: str | None = None) -> dict:
        conn = self._get_conn()
    ...
    def get_prediction_accuracy(self, sport: str | None = None, n: int = 100) -> dict:
        conn = self._get_conn()
    ...
    def get_rating(self, team: str, sport: str, rtype: str = "elo") -> dict:
        conn = self._get_conn()
        row = conn.execute(
            "SELECT * FROM team_ratings WHERE team=? AND sport=? AND rating_type=?", (team, sport, rtype)
        ).fetchone()
        if row:
            return dict(row)
        return {
        ...
        with self.transaction() as conn:
            ...
            """,
                (
                    ...
                    sport,
                    rtype,
                    existing.get("rating", 1500),
                    existing.get("rd", 350),
                    existing.get("volatility", 0.06),
                    existing.get("mu", 25),
                    existing.get("sigma", 8.333),
                    existing.get("matches_played", 0),
    ...
    def upsert_player(self, player: dict):
        pid = player.get("player_id") or hashlib.md5(player["name"].encode()).hexdigest()[:16]
        with self.transaction() as conn:
            conn.execute(
                """
            ...
            """,
                (
                    ...
                    player.get("batting_sr", 0),
                    player.get("bowling_avg", 0),
                    player.get("bowling_econ", 0),
                    player.get("bowling_sr", 0),
                    player.get("fielding_catches", 0),
                    player.get("matches", 0),
                    player.get("runs", 0),
                    player.get("wickets", 0),
                    player.get("impact_rating", 50),
        ...
        with self.transaction() as conn:
            ...
            """,
                (
                    ...
                    odds.get("implied_prob_a", 0.5),
                    odds.get("implied_prob_b", 0.5),
    ...
    def get_odds_history(self, match_id: str) -> list[dict]:
        conn = self._get_conn()
        rows = conn.execute("SELECT * FROM odds_history WHERE match_id=? ORDER BY timestamp", (match_id,)).fetchall()
        return [dict(r) for r in rows]

    # ── Cache ──
    def cache_get(self, key: str) -> Any | None:
        conn = self._get_conn()
        row = conn.execute("SELECT data, ttl_hours, created_at FROM api_cache WHERE cache_key=?", (key,)).fetchone()
        if row:
    ...
    def cache_set(self, key: str, data: Any, ttl_hours: float = 6):
        with self.transaction() as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (key, json.dumps(data, default=str), ttl_hours, datetime.now().isoformat()),
            )
    ...
    def log_event(self, event_type: str, details: dict | None = None):
        with self.transaction() as conn:
            conn.execute(
                "INSERT INTO audit_log (event_type, details_json) VALUES (?,?)", (event_type, json.dumps(details or {}))
            )
...
class RateLimiter:
    ...
    _instances: dict[str, RateLimiter] = {}  # noqa: RUF012
    ...
    def acquire(self, timeout: float = 30.0) -> bool:
        ...
        while True:
            with self._lock_instance:
                ...
                self.tokens = min(self.max_tokens, self.tokens + elapsed * (self.max_tokens / self.period))
...
class RatingEngine:
    ...
    def elo_update(
        ...
        self.db.update_rating(winner, sport, "elo", rating=new_w, matches_played=w["matches_played"] + 1)
        self.db.update_rating(loser, sport, "elo", rating=new_l, matches_played=l["matches_played"] + 1)
    ...
    def trueskill_update(self, winner: str, loser: str, sport: str):
        ...
        try:
            ...
            self.db.update_rating(
                winner, sport, "trueskill", mu=new_w.mu, sigma=new_w.sigma, matches_played=w["matches_played"] + 1
            )
            self.db.update_rating(
                loser, sport, "trueskill", mu=new_l.mu, sigma=new_l.sigma, matches_played=l["matches_played"] + 1
            )
        except ImportError:
            self.elo_update(winner, loser, sport)

    def surface_elo_update(self, winner: str, loser: str, sport: str, surface: str, K: float = 32):
        ...
        self.db.update_rating(winner, sport, rtype, rating=new_w, matches_played=w["matches_played"] + 1)
        self.db.update_rating(loser, sport, rtype, rating=new_l, matches_played=l["matches_played"] + 1)
        return new_w, new_l

    def update_all(self, winner: str, loser: str, sport: str, margin: float = 0, surface: str = "", stage: str = ""):
...
class ProbabilityCalibrator:
    ...
    def fit_platt(self, predicted_probs: np.ndarray, actual_outcomes: np.ndarray):
        ...
        def neg_log_likelihood(params):
            ...
            return -np.mean(actual_outcomes * np.log(calibrated) + (1 - actual_outcomes) * np.log(1 - calibrated))
    ...
    def fit_isotonic(self, predicted_probs: np.ndarray, actual_outcomes: np.ndarray, n_bins: int = 10):
    ...
    def reliability_diagram(self) -> dict:
        ...
        if self._iso_bins is None:
            return {}
        return {"bins": [{"predicted": b[0], "actual": b[1], "count": b[2]} for b in self._iso_bins]}
...
class WalkForwardBacktester:
    ...
    def __init__(self, model_factory: Callable, feature_extractor: Callable, min_training_size: int = 10):
        self.model_factory = model_factory
        self.feature_extractor = feature_extractor
        self.min_training = min_training_size

    def run(self, matches: list[dict], odds_data: dict | None = None) -> BacktestResult:
        """
...
class MonteCarloSimulator:
    ...
    def __init__(self, predict_fn: Callable, n_simulations: int = 10000, seed: int = 42):
        """
    ...
    def simulate_group(self, teams: list[str], fixtures: list[tuple[str, str]]) -> list[tuple[str, int]]:
    ...
    def simulate_tournament(self, groups: dict[str, list[str]], n_qualify: int = 2) -> dict:
        """
        Full tournament simulation.
        groups: {"A": ["team1", "team2", ...], "B": [...], ...}
        """
        results = defaultdict(lambda: {"champion": 0, "finalist": 0, "semifinalist": 0, "qualified": 0})

        for _ in range(self.n_sims):
            # Group stage
            qualifiers = []
            for _group_name, teams in groups.items():
                ...
                for team, _pts in standings[:n_qualify]:
        ...
        return dict(sorted(output.items(), key=lambda x: -x[1]["champion_pct"]))

    def simulate_series(self, team_a: str, team_b: str, best_of: int = 7) -> dict:
...
class BiasAuditor:
    ...
    def audit(predictions: list[dict]) -> dict:
        ...
        sum(1 for p in home_preds if p.get("is_correct"))

        # Favorite bias (did we always pick the favorite?)
        fav_picked = sum(1 for p in scored if p.get("prob_a", 0.5) > 0.6)
        fav_correct = sum(1 for p in scored if p.get("prob_a", 0.5) > 0.6 and p.get("is_correct"))

        # Underdog detection
        upsets = [p for p in scored if not p.get("is_correct") and p.get("prob_a", 0.5) > 0.65]

        # Confidence calibration
        high_conf = [p for p in scored if p.get("prob_a", 0.5) > 0.7 or p.get("prob_a", 0.5) < 0.3]
        ...
        return {
            ...
            "favorite_bias": {
                ...
                if fav_picked / total > 0.7
                else "MODERATE"
                if fav_picked / total > 0.5
                else "LOW",
            },
            "upset_detection": {
            ...
            "stage_accuracy": {
                k: {"accuracy": v["correct"] / v["total"] if v["total"] else 0, **v} for k, v in stage_acc.items()
            },
        }
...
def test_core():
    ...
    print(f"  ✅ Ratings: Elo={r['elo']['rating']:.0f}, Glicko={r['glicko2']['rating']:.0f}")
    ...
    def mock_predict(a, b):
        elo = {"India": 2150, "NZ": 2040, "SA": 2070, "ENG": 2080, "AUS": 2015, "WI": 1950, "PAK": 1920, "SL": 1854}
        ea = elo.get(a, 1500)
        eb = elo.get(b, 1500)
        pa = 1 / (1 + 10 ** ((eb - ea) / 400))
        return pa, 1 - pa
