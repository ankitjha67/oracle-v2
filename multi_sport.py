...
class SportElo:
    def __init__(self, default=1500, K=25, home_adv=50):
        ...
        self._rating_callback = None  # Optional: callable(team, rating) for history tracking
...
def _parse_fight_event(event):
    ...
    for comp in event.get("competitions", []):
        ...
        a0 = f0.get("athlete", {})
        a1 = f1.get("athlete", {})
        n0 = a0.get("displayName", "?")
        n1 = a1.get("displayName", "?")
        if n0 == "?" and n1 == "?":
            continue
        ...
        w0 = f0.get("winner", False)
        w1 = f1.get("winner", False)
        status = comp.get("status", {}).get("type", {}).get("name", "")
        fights.append(
            {
                "fighter_a": n0,
...
def build_team_sport(sport_key, days_back=30, days_ahead=7):
    ...
    for d in range(days_back, -1, -1):
        ...
        for ev in data.get("events", []):
            ...
            if not p or "FINAL" not in p["status"].upper():
                continue
...
def build_ufc(days_back=60, days_ahead=14):
    ...
    for d in range(0, days_ahead + 1):
        ...
        for ev in data.get("events", []):
            for fight in _parse_fight_event(ev):
                if fight["status"] and "SCHEDULED" not in fight["status"].upper():
                    continue
...
def build_f1():
    ...
    driver_list = [
        {"number": d.get("driver_number"), "name": d.get("full_name", "?"), "team": d.get("team_name", "?")}
        for d in drivers
        if d.get("full_name")
    ]
    return {
...
def run_all_sports(sport_keys=None, days_back=30, days_ahead=7):
    ...
    for sport in sport_keys:
        ...
        for p in result.get("predictions", [])[:8]:
            pr = p["prediction"]
            ic = "🟠" if pr["confidence"] == "HIGH" else "🟡" if pr["confidence"] == "MODERATE" else "🟢"
            rec = f" [{p['records']['a']} vs {p['records']['b']}]" if "records" in p else ""
            print(
                f"      {p['date']} {p['match']:<45} {ic} {pr['winner']:<20} ({pr['prob_a']:.0f}-{pr['prob_b']:.0f}){rec}"
            )
        if n > 8:
            print(f"      ... +{n - 8} more")
